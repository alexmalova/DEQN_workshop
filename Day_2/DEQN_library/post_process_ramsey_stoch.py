...
starting_state = tf.reshape(tf.constant([
    Parameters.k0, Parameters.zeta_log0, Parameters.chi0,
    Parameters.zetatilde_log0,
    # Pseudo states
    Parameters.rho0, Parameters.gamma0, Parameters.psi0, 
...
for pidx, ps in enumerate(Parameters.policy_states):
    ...
    if ps in ['con_tildey', 'inv_tildey']:
        tfp = Definitions.tfp(state_1episode, policy_state_1episode)
        lab = Definitions.lab(state_1episode, policy_state_1episode)
...
for pidx, policy in enumerate(Parameters.policy_states):
    ...
    for tidx in range(N_episode_length):
        ...
        if policy in ['con_tildey', 'inv_tildey']:
            tfp = Definitions.tfp(state_batch, policy_state_batch)
            lab = Definitions.lab(state_batch, policy_state_batch)
...
for didx, de in enumerate(econ_defs):
    ...
    for tidx in range(N_episode_length):
        ...
        elif de in ['vnorm_tilde']:
            # Multiply tfp * lab
            zeta = Definitions.zeta(state_batch, policy_state_batch)
            zeta_tilde = Definitions.zeta_tilde(
                state_batch, policy_state_batch)
            defined_val = zeta / zeta_tilde * defined_val
...
#       "distributions. This section should be commented out for the Cai and "
#       "Lontzek (2019).")
# # --------------------------------------------------------------------------- #
# # For the UQ analysis, the starting state of the all pseudo states must be
# # different. Here we populate random pseudo states where all numbers are drawn
# # from the corresponding uniform distribution.
...
# # starting state
# pseudo_states = ['rhox', 'gammax', 'psix', 'rx', 'varrhox', 'varsigmax',
#                  'f2xco2x', 't2xco2x', 'epsscalex']
# N_pseudos = len(pseudo_states)
# pseudo_batch = np.random.uniform(
#     [Parameters.rho_lower, Parameters.gamma_lower, 1.25,
#      Parameters.r_lower, Parameters.varrho_lower,
#      Parameters.varsigma_lower, Parameters.f2xco2_lower,
#      Parameters.t2xco2_lower, Parameters.epsscale_lower],
#     [Parameters.rho_upper, Parameters.gamma_upper, Parameters.psi_upper,
#      Parameters.r_upper, Parameters.varrho_upper,
#      Parameters.varsigma_upper, Parameters.f2xco2_upper,
#      Parameters.t2xco2_upper, Parameters.epsscale_upper],
#     [N_sim_batch, N_pseudos])
#
